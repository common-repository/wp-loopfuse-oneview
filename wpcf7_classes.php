...
class WPCF7_ContactForm {
	...
	/* LoopFuse OneView Function */
	
	function get_loopfuse_form_elements($cf7action) 
	{
		global $wpdb, $form_NAME, $form_ID, $form_ACTION;
		...
		$sql = "SELECT cid FROM $table_loopfuse_cid WHERE id= '1'"; 
		$lf_cid = $wpdb->get_var($sql); 

		$sql = "SELECT * FROM $table_name WHERE cf7_id= '$cf7_id' and use_lf_form = '1'"; 

		$lf_form = $wpdb->get_row($sql); 
		$formId = $lf_form->formId;  
		$formName = $lf_form->formName;  
		$formAction = $lf_form->formAction;
	
	
		if(count($lf_form) == 1) 
		{
			$html .= '<!-- LOOPFUSE ELEMENTS START -->'. "\n";
			$html .= '<input type="hidden" name="loopfuse" value="1" />' . "\n";			
			$html .= '<input type="hidden" name="formAction" value="'.trim($formAction).'" />' . "\n"; 
			$html .= '<input type="hidden" name="formid" value="'.$formId.'" />' . "\n"; 
			$html .= '<input type="hidden" name="cid" value="'.$lf_cid.'" />' . "\n";
			$html .= '<input type="hidden" name="cf7action" value="'.trim($cf7action).'" />' . "\n";
			$html .= '<!-- LOOPFUSE ELEMENTS END   -->'. "\n";
		}			
		return $html;
	}
	...
	function form_html() {
		...
		$form .= '<form name="cf7form" action="' . esc_url_raw( $url ) . '" method="post"'
			. ' class="' . esc_attr( $class ) . '"' . $enctype . '>' . "\n";
		
		$form .= $this->get_loopfuse_form_elements(esc_url_raw( $url ));
